variable "resource_group_name" {
  description = "(Required) Name of existing resource group to deploy resources"
  type        = string
}

variable "virtual_network_name" {
  description = "(Required) Name of existing virtual network to deploy subnets to"
  type        = string
}
  ...
  type = map(object(
    {
      ...
      private_endpoint_network_policies_enabled     = optional(bool, false) # (Optional) Enable or Disable network policies for the private endpoint on the subnet. Setting this to `true` will **Enable** the policy and setting this to `false` will **Disable** the policy. Defaults to `false`.
